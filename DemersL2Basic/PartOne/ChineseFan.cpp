...
void init() {
	...
	//glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(fanColors), fanColors);
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 0, 0);

	//Not sure if needed, for indices to allow blending
	//glGenBuffers(1, &ebo); //Global variable GLuint ebo
...
void display(void) {
	...
	//Need to update the vertices to allow the fan to fold
	if (vertex_update) {
		glBindBuffer(GL_ARRAY_BUFFER, vbo[0]); //This buffer allows the triangles to have unique colors
		glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices_update), vertices_update);
	}
	else {
		glBindBuffer(GL_ARRAY_BUFFER, vbo[0]); //This buffer does the same as the one in the 'if'
		glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices), vertices);
